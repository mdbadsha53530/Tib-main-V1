...
if (!wall) {
  var bx = [[{ title: "💼 Set/ Change Wallet", command: "/sewa" }]]
  let walle = Bot.getProperty("wallet" + user.telegramid)

  if (!walle) {
    User.setProperty("walletuserid", "not set", "string")
  } else {
  ...
  if (message == "🔙 Back") {
    ...
    if (!isNumeric(value)) {
      Bot.sendMessage(
        "❌ Send a Value over " + min + ", and under (or Max) " + btc.value()
      )
    } else {
      ...
      if (btc.value() >= min && amount >= min && amount <= btc.value()) {
        ...
        if (duration_in_hours < 1) {
          var mgs =
            "_You already made and withdrawan. Please come back again after_ *"
        ...
        if (aw != "enabled") {
          ...
          let megay = {
            ...
            bd_time: new Date().toLocaleString("en-US", {
              timeZone: "Africa/Addis_Ababa" //edit your country timezone here
            }),
            withdrawal_info: {
              wd_amount: message,
              wd_time: timenow,
          ...
          Bot.setProperty(
            "m_requestid" + user.telegramid + wdlist2.value(),
            request.message_id,
            "integer"
          )
          Bot.setProperty(
            "wd" + user.telegramid + "-" + wdlist2.value(),
            megay,
            "json"
          )
          push.push(megay)
          ...
          if (adm_id) {
            let tcx =
              ...
              "</code>"
            Bot.setProperty(
              "wdstatus" + user.telegramid + wdlist2.value(),
              "⏱ Pending",
              "string"
            )
            var buttons = [
          ...
          let megax = {
            ...
            w_id: wdlist2.value(),
            bd_time: new Date().toLocaleString("en-US", {
              timeZone: "Asia/Dhaka" //edit your country timezone here
            }),
            withdrawal_info: {
              wd_amount: message,
              wd_time: timenow,
